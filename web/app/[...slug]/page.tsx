...
const tradeData: Record<string, { title: string; keyword: string }> = {
    ...
    'aviation-maintenance': { title: 'Aviation Maintenance', keyword: 'Aviation' },
    'commercial-driving-cdl': { title: 'Commercial Driving (CDL)', keyword: 'Driving' },
    'culinary-arts': { title: 'Culinary Arts', keyword: 'Culinary' },
    'solar-energy-technology': { title: 'Solar Energy Technology', keyword: 'Solar' },
};
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    // 3. Logic C: City Trade Page (/[state]/[city]/[trade]) e.g., /tx/houston/welding
    if (slug.length === 3) {
        const state = slug[0].toLowerCase();
        const city = slug[1].split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' '); // Simple un-slugify
        const tradeKey = slug[2].toLowerCase();
        const tradeDataEntry = tradeData[tradeKey];

        if (STATE_MAP[state] && tradeDataEntry) {
            return {
                title: `Best ${tradeDataEntry.title} Schools in ${city}, ${state.toUpperCase()} (${SEO_YEAR})`,
                description: `Compare top rated ${tradeDataEntry.title} programs in ${city}, ${state.toUpperCase()}. Verified tuition costs, salaries, and ROI for ${SEO_YEAR}.`,
            };
        }
    }
